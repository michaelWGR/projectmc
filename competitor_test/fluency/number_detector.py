...
def get_roi_frame(frame, roi_type):
    shape = frame.shape
    assert shape[0] in (720, 1080) or shape[0] in (750,1334)
...
def main():
    ...
    for video_or_frame_dir in args.videos_or_frame_dirs:
        ...
        if os.path.isfile(video_or_frame_dir) and do_extract:
            # extract_frames

            video_path = video_or_frame_dir
            video_file_name = os.path.basename(video_path)
            video_name = os.path.splitext(video_file_name)[0]

            frame_rate = args.frame_rate
            print('extract frames from %s, %f fps' % (video_path, frame_rate))
            frame_dir = extract_frames(video_path, frame_rate, output_base_dir=output_base_dir, frame_type=frame_type)
            if distinct:
                print('duplicate %s' % (frame_dir))
                duplication(frame_dir, frame_type=frame_type)
        else:
            if os.path.isfile(video_or_frame_dir):
                frame_dir = get_frame_dir(video_or_frame_dir, output_base_dir=output_base_dir)
            else:
                frame_dir = video_or_frame_dir
            video_name = os.path.basename(frame_dir)
        ...
        if do_roi:
            # roi
            print('write roi from %s' % (frame_dir, ))
        ...
        if do_detect:
            if not do_roi:
                roi_frame_dir = get_roi_frame_dir(frame_dir, output_base_dir=output_base_dir)
                roi_frame_filepaths = get_frame_filepaths(roi_frame_dir, frame_type=frame_type)

            print('tesseract %s to %s' % (roi_frame_dir, output_csv_path))
            with tesseract.temp(), open(output_csv_path, 'wb') as csv_file:
                csv_writer = csv.writer(csv_file)
                for roi_frame_filepath in roi_frame_filepaths:
                    tesseract_result = tesseract.parse_by_file(roi_frame_filepath)

                    frame_name = os.path.basename(roi_frame_filepath)
                    csv_writer.writerow([frame_name, tesseract_result])
