...
const int CV_DEFAULT_BINARY_THRESH = 200;
const int CV_DEFAULT_CANNY_THRESH1 = 100;
const int CV_DEFAULT_CANNY_THRESH2 = 200;
...
float cross(const Point2f &v1, const Point2f &v2);
...
bool marker_less(const Marker &m1, const Marker &m2);
...
void print_usage(const char* usage_name);

int fluency_detect(const char* image_path, Options options);

// Start of Main Loop
//------------------------------------------------------------------------------------------------------------------------

void print_usage(const char* usage_name) {
    cout << "Usage: " << usage_name << " [OPTION]... <IMAGE>" << endl
         << "Fluency detection from each IMAGE file." << endl
         << endl
         << "Options:" << endl
         << "  (-d|--debug)              debug" << endl
         << "  (-bt|--binary-threshold)  threshold for binary, default is 200" << endl
         << "  (-ct1|--canny-threshold1) first threshold for canny, default is 100" << endl
         << "  (-ct2|--canny-threshold2) second threshold for canny, default is 200" << endl
         << endl
         << "Example usage:" << endl
         << "  " << usage_name << " -d example.bmp" << endl
         << endl;
...
int main(int argc, char **argv) {
    ...
    options.binary_threshold = CV_DEFAULT_BINARY_THRESH;
    options.canny_threshold1 = CV_DEFAULT_CANNY_THRESH1;
    options.canny_threshold2 = CV_DEFAULT_CANNY_THRESH2;
...
//            if (0 == strcmp(arg, "-do") || 0 == strcmp(arg, "--debug-output")) {
//
//            }

            if (0 == strcmp(arg, "-bt") || 0 == strcmp(arg, "--binary-threshold")) {
                char* value = argv[i];
                i++;

                options.binary_threshold = atoi(value);
            }

            if (0 == strcmp(arg, "-ct1") || 0 == strcmp(arg, "--canny-threshold1")) {
                char* value = argv[i];
                i++;

                options.canny_threshold1 = atoi(value);
            }

            if (0 == strcmp(arg, "-ct2") || 0 == strcmp(arg, "--canny-threshold2")) {
                char* value = argv[i];
                i++;

                options.canny_threshold2 = atoi(value);
            }
    ...
    if (NULL == image_path) {
        print_usage("fluency_detection");
    }

    return fluency_detect(image_path, options);
}
...
int fluency_detect(const char* image_path, Options options) {
    ...
    int DBG = options.debug;

    traces = Scalar(0, 0, 0);
    qr_raw = Mat::zeros(480, 980, CV_8UC3); // Mat::zeros(100, 100, CV_8UC3);
    qr = Mat::zeros(480, 980, CV_8UC3); // Mat::zeros(100, 100, CV_8UC3 );
    qr_roi = Mat::zeros(320, 820, CV_8UC3);
    qr_gray = Mat::zeros(480, 980, CV_8UC1); // Mat::zeros(100, 100, CV_8UC1);
    qr_thres = Mat::zeros(480, 980, CV_8UC1); // Mat::zeros(100, 100, CV_8UC1);

    cvtColor(image, gray, CV_BGR2GRAY);        // Convert Image captured from Image Input to GrayScale

    blur(gray, gray, Size(3, 3));  // denoising
    threshold(gray, gray, options.binary_threshold, 255, THRESH_BINARY);
    Canny(gray, edges_raw, options.canny_threshold1, options.canny_threshold2, 5);

    edges_raw.copyTo(edges);
    ...
    for (int i = 0; i < contours.size(); i++) {
        ...
        if (c >= 5) {
            ...
            m.index = i;
            m.deep = c;
            markers.insert(markers.end(), m);
        }
    }

    sort(markers.begin(), markers.end(), marker_less);

    // TODO if there are more than 3?
    for (vector<Marker>::iterator it = markers.begin(); it != markers.end(); it++) {
        if (mark == 0) A = it->index;
        else if (mark == 1) B = it->index;        // i.e., A is already found, assign current contour to B
        else if (mark == 2) C = it->index;        // i.e., A and B are already found, assign current contour to C
        mark = mark + 1;
    }
    ...
    if (mark >= 3) {
        ...
        if (top < contours.size() && right < contours.size() && bottom < contours.size() &&
            contourArea(contours[top]) > 10 && contourArea(contours[right]) > 10 &&
            ...
            if (src.size() == 4 && dst.size() == 4) {
                ...
                if (DBG == 1) {
                    ...
                    qr_raw(cv::Rect(80, 80, 820, 320)).copyTo(qr_roi);
                    copyMakeBorder(qr_roi, qr, 10, 10, 10, 10, BORDER_CONSTANT, Scalar(0, 0, 0));
    ...
    if (DBG == 1) {
//        imwrite("/Users/libingxi/tmp/debug/traces.bmp", traces);
//        imwrite("/Users/libingxi/tmp/debug/edges.bmp", edges_raw);
        imshow("Image", image);
        imshow("Traces", traces);
        imshow("Edges", edges_raw);
        imshow("QR code", qr_thres);
        waitKey(0);    // OPENCV: wait for 1ms before accessing next frame
    }
