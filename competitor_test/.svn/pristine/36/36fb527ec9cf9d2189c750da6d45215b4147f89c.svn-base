...
video_frame_paths = []
mobile_threshold = 0.1
game_live_threshold = 38.0
result = []
...
def extarct_soundtrack(arg_video_path):
    sound_lists = os.listdir(arg_video_path)
    for line in sound_lists:
        if line.endswith('.avi') or line.endswith('.mp4') or line.endswith('.mov'):
            cmd = [os.path.join(ffmpeg_default_path, 'ffmpeg'), '-i', os.path.join(arg_video_path, line), '-acodec', 'copy', '-vn', os.path.join(arg_video_path, os.path.splitext(line)[0] + '.wav')]
            child = subprocess.Popen(cmd, stdin=subprocess.PIPE, stdout=subprocess.PIPE, cwd=ffmpeg_default_path)
            child.wait()
...
def detect_first_frame_gray(arg_frame_path):
    ...
    tag = True
    if is_mobile:
        seq = int(bit_rate) * 4 + 1
        while seq < len(frames):
            img = cv2.imread(frames[seq], cv2.IMREAD_GRAYSCALE)
        ...
        while seq < len(frames):
            ...
            if tag:
                ...
                if caculate_gray_value(resize_img) > game_live_threshold:
                    video_delay[i] = float(seq + 1)/float(bit_rate)
                    i += 1
                    seq += int(bit_rate)*9
                    tag = False
            else:
                print seq
                if caculate_gray_value(resize_img) < game_live_threshold:
                    video_delay[j] = float(seq + 1)/float(bit_rate)
                    j += 1
                    seq += int(bit_rate)*4
                    tag = True
            seq += 1
